...
(function() {
    ...
    xhttp.onreadystatechange = function() {
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            images = JSON.parse(xhttp.responseText).images;
            filterImages();
        }
    };
...
function filterImages(filter) {
    ...
    for (var i = 0; i < images.length; i++) {
        ...
        newHTML += "<div class=\"pic\" style=\"background: url('"+((images[i].thumb=="")?images[i].img:images[i].thumb)+"') 50% 50% no-repeat; background-size: contain; background-size: cover;\"></div></div>";
    }
